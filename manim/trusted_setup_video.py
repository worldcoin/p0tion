...
def txt(
    ...
    **kwargs,
    ...
    return Text(s, font_size=size, color=_color(color), font=font, **kwargs)
...
def card(title: str, body: str, *, w: float = 4.0, h: float = 1.6, color: str = PAL.blue) -> VGroup:
    r = RoundedRectangle(
        width=w,
        height=h,
        corner_radius=0.15,
...
class BaseTrustedSetupScene(Scene):
    ...
    def scene_title(
        ...
        *,
        ...
        corner_box = None
        if corner:
            corner_box = label_box(corner, font_size=22, color=corner_color).to_corner(UR, buff=0.25)
...
class Scene01_Groth16Basics(BaseTrustedSetupScene):
    def construct(self):
        ...
        teaser.scale(0.7).to_edge(LEFT, buff=0.42).shift(DOWN * 1.25)
        self.play(FadeIn(title, shift=DOWN * 0.05), run_time=RT_TITLE_IN)
        self.play(FadeIn(teaser, shift=DOWN * 0.12), run_time=RT_BEAT)
        self.wait(0.5)
        self.play(FadeOut(teaser, shift=UP * 0.06), run_time=RT_BEAT)
        self.wait(0.25)
        ...
        self.play(FadeIn(statement, shift=UP * 0.08), run_time=RT_IN)
        self.wait(0.25)

        prover = RoleToken("Prover", color=PAL.blue).move_to(LEFT * 4.2 + DOWN * 1.45)
        verifier = RoleToken("Verifier", color=PAL.blue).move_to(RIGHT * 4.2 + DOWN * 1.45)
        channel = Arrow(
        ...
        secret = label_box("Password (secret)", font_size=24, color=PAL.amber).next_to(prover, DOWN, buff=0.45)
        self.play(
        ...
        caption = txt("Verifier accepts, but never sees the password.", size=19, color=PAL.muted).to_edge(DOWN, buff=0.62)
        self.play(secret.animate.set_opacity(0.18), FadeIn(caption, shift=UP * 0.05), run_time=RT_BEAT)
        self.wait(0.6)
        self.play(FadeOut(caption, shift=DOWN * 0.06), run_time=RT_BEAT)
        self.wait(0.2)

        proof = ProofTicket("Proof", color=PAL.teal).scale(0.88).move_to(LEFT * 1.75 + DOWN * 0.45)
        self.play(TransformFromCopy(statement, proof), run_time=RT_BEAT)
        self.wait(0.2)
        self.play(
            proof.animate.move_to(RIGHT * 1.35 + DOWN * 0.45),
            run_time=RT_BEAT,
        )

        accept = label_box("ACCEPT", font_size=26, color=PAL.green).next_to(verifier, UP, buff=0.45)
        self.play(FadeIn(accept, shift=DOWN * 0.08), run_time=RT_BEAT)
        self.wait(0.5)

        phase_demo = VGroup(statement, secret, proof, accept, prover, verifier, channel)
        self.play(FadeOut(phase_demo, shift=DOWN * 0.06), run_time=RT_OUT)
        self.wait(0.25)

        d_statement = label_box("Statement: what you claim", font_size=22, color=PAL.blue)
        d_witness = label_box("Witness: the secret", font_size=22, color=PAL.amber)
        d_proof = label_box("Proof: convinces without revealing", font_size=22, color=PAL.teal)
        defs = VGroup(d_statement, d_witness, d_proof).arrange(DOWN, buff=0.22).move_to(DOWN * 0.35)

        self.play(FadeIn(d_statement, shift=UP * 0.05), run_time=RT_IN)
        self.play(FadeIn(d_witness, shift=UP * 0.05), run_time=RT_IN)
        self.play(FadeIn(d_proof, shift=UP * 0.05), run_time=RT_IN)
        self.wait(0.6)

        self.play(FadeOut(defs, shift=DOWN * 0.06), run_time=RT_OUT)
        self.wait(0.25)

        setup = label_box("Some proof systems need a one-time setup.", font_size=26, color=PAL.teal)
        setup.move_to(DOWN * 0.25)
        self.play(FadeIn(setup, shift=DOWN * 0.08), run_time=RT_IN)
        self.wait(0.9)
...
class Scene02_CRSArtifacts(BaseTrustedSetupScene):
    def construct(self):
        ...
        crs_label = txt("Setup parameters (public)", size=28, color=PAL.text).next_to(crs, UP, buff=0.12)

        split = Line(crs.get_top() + DOWN * 0.22, crs.get_bottom() + UP * 0.22).move_to(crs.get_center())
        ...
        vk_box = pk_box.copy().set_stroke(color=_color(PAL.blue)).move_to(crs.get_center() + RIGHT * 2.05 + UP * 0.1)
        ...
        pk_note = label_box("Used by the prover", font_size=22, color=PAL.teal).next_to(pk_box, DOWN, buff=0.24)
        vk_note = label_box("Used by the verifier", font_size=22, color=PAL.blue).next_to(vk_box, DOWN, buff=0.24)
        ...
        self.play(FadeIn(pk, shift=UP * 0.04), FadeIn(vk, shift=UP * 0.04), run_time=RT_IN)
        self.play(FadeIn(pk_note, shift=UP * 0.05), FadeIn(vk_note, shift=UP * 0.05), run_time=RT_IN)

        note = txt("These files are published for everyone.", size=23, color=PAL.muted).to_edge(DOWN, buff=0.42)
        self.play(FadeIn(note, shift=UP * 0.05), run_time=RT_IN)
        self.wait(0.6)

        self.transition_out(title, crs, crs_label, split, pk_box, vk_box, pk, vk, pk_note, vk_note, note)
...
class Scene03_ToxicWaste(BaseTrustedSetupScene):
    def construct(self):
        ...
        vial_lbl = txt("Setup\nsecret", size=28, color=PAL.red, line_spacing=0.85).move_to(vial.get_center())
        vial_hint = txt("(must be destroyed)", size=22, color=PAL.muted).next_to(vial, DOWN, buff=0.2)

        safe = label_box("If destroyed", font_size=24, color=PAL.green).shift(RIGHT * 1.35 + UP * 1.1)
        bad = label_box("If kept", font_size=24, color=PAL.red).shift(RIGHT * 1.35 + DOWN * 0.15)

        self.play(Create(vial), Write(vial_lbl), FadeIn(vial_hint, shift=UP * 0.05), run_time=RT_IN)
        self.play(FadeIn(safe, shift=DOWN * 0.12), FadeIn(bad, shift=UP * 0.12), run_time=RT_IN)
        ...
        honest = ProofTicket(
            ...
            height=1.0,
            font_size=24,
        ).scale(0.9).next_to(safe, RIGHT, buff=0.92)
        forged = ProofTicket(
            "Proof can be forged",
            color=PAL.red,
            width=3.9,
            height=1.0,
            font_size=24,
        ).scale(0.9).next_to(bad, RIGHT, buff=0.92)

        self.play(FadeIn(honest, shift=LEFT * 0.12), run_time=RT_IN)
        self.play(FadeIn(forged, shift=LEFT * 0.12), run_time=RT_IN)

        same = txt("They can look identical to the verifier.", size=23, color=PAL.muted).to_edge(DOWN, buff=0.48)
        ...
        forgery_note = txt("Risk: forged proofs if setup secret survives.", size=28, color=PAL.red)
        notes = VGroup(n0, privacy_note, forgery_note).arrange(DOWN, aligned_edge=LEFT, buff=0.2).move_to(RIGHT * 1.9 + DOWN * 0.05)
        box = SurroundingRectangle(notes, color=_color(PAL.blue), buff=0.28, corner_radius=0.15)
        self.play(FadeIn(box, shift=UP * 0.05), FadeIn(notes, shift=UP * 0.05), run_time=RT_IN)
        self.wait(0.7)
...
class Scene04_UpdatableMPC(BaseTrustedSetupScene):
    def construct(self):
        title, _ = self.scene_title("Many contributors (the core trick)", subtitle="If one person is honest, we're safe.")
        ...
        state = label_box("Parameter state: P0", font_size=22, color=PAL.blue).to_edge(DOWN).shift(UP * 0.5)

        self.play(
            LaggedStart(*[FadeIn(c, shift=UP * 0.08) for c in contributors], lag_ratio=0.12),
            Create(inbound),
            Create(connector_01),
            Create(connector_12),
        ...
        for i, c in enumerate(contributors, start=1):
            r_txt = txt(f"secret r{i}", size=24, color=PAL.amber).next_to(c, UP, buff=0.18)
            destroy_tag = label_box("destroyed", font_size=18, color=PAL.red).next_to(c, DOWN, buff=0.2)
            incoming_pos = c.get_left() + RIGHT * 0.24
            outgoing_pos = c.get_right() + LEFT * 0.24
            next_state = label_box(f"Parameter state: P{i}", font_size=22, color=PAL.blue).move_to(state)

            self.play(param.animate.move_to(incoming_pos), FadeIn(r_txt, shift=DOWN * 0.05), run_time=RT_BEAT)
        ...
        concl_box = SurroundingRectangle(conclusion, color=_color(PAL.teal), buff=0.25, corner_radius=0.15)
        self.play(FadeIn(concl_box), Write(conclusion), run_time=RT_IN)
        self.wait(0.9)

        self.transition_out(title, *contributors, inbound, connector_01, connector_12, param, conclusion, concl_box)
...
class Scene05_TranscriptChain(BaseTrustedSetupScene):
    def construct(self):
        title, _ = self.scene_title("Publicly verifiable", subtitle="Transcript chain + checks (without revealing secrets)")
        self.play(Write(title), run_time=RT_TITLE_IN)
        self.wait(0.35)
        ...
        arrows = VGroup(
            Arrow(chain[0].get_right(), chain[1].get_left(), buff=0.15, stroke_width=2.75, color=_color(PAL.muted)),
            Arrow(chain[1].get_right(), chain[2].get_left(), buff=0.15, stroke_width=2.75, color=_color(PAL.muted)),
        )

        self.play(LaggedStart(*[Create(b) for b in chain], lag_ratio=0.15), run_time=RT_IN)
        self.play(LaggedStart(*[GrowArrow(a) for a in arrows], lag_ratio=0.2), run_time=RT_BEAT)

        check = label_box("Anyone can verify the chain links + proofs check out", font_size=24, color=PAL.teal).to_edge(DOWN)
...
class Scene06_Phase1Phase2Pipeline(BaseTrustedSetupScene):
    def construct(self):
        title, _ = self.scene_title("Two phases of setup", subtitle="Universal setup, then circuit-specific setup")
        self.play(Write(title), run_time=RT_TITLE_IN)
        self.wait(0.35)
        ...
        a1 = Arrow(phase1.get_right(), ptau.get_left(), buff=0.15, stroke_width=2.75, color=_color(PAL.muted))
        a2 = Arrow(circuit.get_right(), phase2.get_left(), buff=0.15, stroke_width=2.75, color=_color(PAL.muted))
        a3 = Arrow(phase2.get_right(), zkey.get_left(), buff=0.15, stroke_width=2.75, color=_color(PAL.muted))
        cross = DashedLine(ptau.get_bottom(), phase2.get_top(), stroke_width=2, color=_color(PAL.muted)).set_opacity(0.6)
        cross_lbl = txt("Phase 2 uses Phase 1 output", size=22, color=PAL.muted).next_to(cross, RIGHT, buff=0.2)

        self.play(FadeIn(diagram, shift=UP * 0.1), run_time=RT_IN)
        self.play(LaggedStart(GrowArrow(a1), GrowArrow(a2), GrowArrow(a3), lag_ratio=0.15), run_time=RT_BEAT)
...
class Scene07_WorldIDPivot(BaseTrustedSetupScene):
    def construct(self):
        title, _ = self.scene_title("World ID 4.0 (case study)", subtitle="Multiple Groth16 circuits, each needs Phase 2 parameters")
        self.play(Write(title), run_time=RT_TITLE_IN)
        self.wait(0.35)

        stack_title = txt("OPRF-service circuits", size=30, color=PAL.text)
        circuits = [
            label_box("OPRFQueryProof", font_size=22, color=PAL.blue),
            label_box("OPRFNullifierProof", font_size=22, color=PAL.blue),
            label_box("OPRFKeyGenProof13", font_size=22, color=PAL.blue),
            label_box("OPRFKeyGenProof25", font_size=22, color=PAL.blue),
            label_box("OPRFKeyGenProof37", font_size=22, color=PAL.blue),
        ]
        stack = VGroup(*circuits).arrange(DOWN, buff=0.18)
        stack_group = VGroup(stack_title, stack).arrange(DOWN, buff=0.25).shift(LEFT * 3.6 + DOWN * 0.2)

        icons = VGroup(
            label_box("R1CS", font_size=22, color=PAL.amber),
            label_box("WASM", font_size=22, color=PAL.amber),
        ).arrange(RIGHT, buff=0.25).next_to(stack, RIGHT, buff=0.8).shift(UP * 0.6)

        anchor = VGroup(
            label_box("Repro anchor", font_size=24, color=PAL.teal),
            txt("source commit + circom version + sha256", size=22, color=PAL.muted),
        ).arrange(DOWN, buff=0.2).shift(RIGHT * 3.0 + DOWN * 0.2)
...
class Scene08_p0tionPrimer(BaseTrustedSetupScene):
    def construct(self):
        ...
        who = VGroup(
            label_box("Who built it: PSE", font_size=24, color=PAL.blue),
            txt("Privacy and Scaling Explorations\n(Ethereum ecosystem R&D)", size=21, color=PAL.muted, line_spacing=0.9),
        ).arrange(DOWN, aligned_edge=LEFT, buff=0.18).shift(LEFT * 4.0 + DOWN * 0.25)
        why = VGroup(
            label_box("Why they built it", font_size=24, color=PAL.teal),
            txt("To coordinate many contributors,\nreduce ceremony mistakes,\nand keep everything auditable.", size=21, color=PAL.muted, line_spacing=0.9),
        ).arrange(DOWN, aligned_edge=LEFT, buff=0.18).shift(ORIGIN + DOWN * 0.25)
        how = VGroup(
            label_box("How p0tion works", font_size=24, color=PAL.amber),
            txt("Queue circuits -> contribute entropy ->\nverify transcript -> finalize artifacts.", size=21, color=PAL.muted, line_spacing=0.9),
        ).arrange(DOWN, aligned_edge=LEFT, buff=0.18).shift(RIGHT * 4.0 + DOWN * 0.25)

        flow = VGroup(
            label_box("Coordinator", font_size=22, color=PAL.teal),
            label_box("Participants", font_size=22, color=PAL.blue),
            label_box("Observers", font_size=22, color=PAL.muted),
        ).arrange(RIGHT, buff=0.35).to_edge(DOWN, buff=0.8)
        ...
        self.transition_out(title, who, why, how, flow)


class Scene08_p0tionWorkflow(BaseTrustedSetupScene):
    def construct(self):
        title, _ = self.scene_title("Ceremony workflow (p0tion-style)", subtitle="Coordinator + participants + observers")
        self.play(FadeIn(title, shift=DOWN * 0.05), run_time=RT_TITLE_IN)
        self.wait(0.35)
        ...
        ).arrange(RIGHT, buff=0.52).shift(UP * 1.66)
        ...
        ).arrange(DOWN, buff=0.22).shift(LEFT * 4.45 + DOWN * 0.88)
        ...
        ).arrange(DOWN, aligned_edge=LEFT, buff=0.14).shift(RIGHT * 2.65 + DOWN * 0.9)
        steps_box = SurroundingRectangle(steps, color=_color(PAL.blue), buff=0.34, corner_radius=0.15)

        cli = VGroup(
            txt("world-id-trusted-setup-cli auth", size=15, color=PAL.muted, font=FONT_MONO),
            txt("world-id-trusted-setup-cli contribute", size=15, color=PAL.muted, font=FONT_MONO),
            txt(
        ...
        ).arrange(DOWN, aligned_edge=LEFT, buff=0.11).to_edge(DOWN, buff=0.22)

        self.play(FadeIn(roles, shift=DOWN * 0.08), run_time=RT_IN)
        self.play(FadeIn(queue, shift=RIGHT * 0.08), run_time=RT_IN)
        self.play(FadeIn(steps_box, shift=UP * 0.04), FadeIn(steps, shift=UP * 0.06), run_time=RT_IN)
...
class Scene09_FinalizationArtifacts(BaseTrustedSetupScene):
    def construct(self):
        title, _ = self.scene_title("Finalization", subtitle="Lock transcript with public randomness, then derive final artifacts")
        self.play(Write(title), run_time=RT_TITLE_IN)
        self.wait(0.35)

        transcript = label_box("Final transcript state", font_size=24, color=PAL.blue).shift(LEFT * 4.35 + DOWN * 0.25)
        beacon = label_box("Public random beacon hash", font_size=23, color=PAL.amber).shift(LEFT * 0.7 + DOWN * 0.25)
        shelf = RoundedRectangle(
            ...
            fill_color=_color(PAL.bg),
            fill_opacity=0.08,
        ).shift(RIGHT * 3.55 + DOWN * 0.25)
        items = VGroup(
            label_box("final.zkey", font_size=24, color=PAL.teal),
            label_box("verification_key.json", font_size=24, color=PAL.blue),
            label_box("verifier.sol", font_size=24, color=PAL.blue),
        ).arrange(DOWN, buff=0.25).move_to(shelf.get_center())
        shelf_label = txt("Deterministic outputs", size=21, color=PAL.muted).next_to(shelf, UP, buff=0.12)

        a1 = Arrow(transcript.get_right(), beacon.get_left(), buff=0.15, stroke_width=2.5, color=_color(PAL.muted)).set_opacity(0.6)
        a2 = Arrow(beacon.get_right(), shelf.get_left(), buff=0.15, stroke_width=2.5, color=_color(PAL.muted)).set_opacity(0.6)
        ...
        self.play(Create(shelf), GrowArrow(a2), FadeIn(items, shift=UP * 0.08), FadeIn(shelf_label, shift=UP * 0.04), run_time=RT_IN)

        stamp = label_box("FINALIZED", font_size=26, color=PAL.green).rotate(0.12).move_to(shelf.get_corner(UR) + LEFT * 1.2 + DOWN * 0.3)
        beacon_hash = txt(
        ...
        self.transition_out(title, transcript, beacon, a1, a2, shelf, shelf_label, items, stamp, beacon_hash)
...
class Scene10_AuditChecklist(BaseTrustedSetupScene):
    def construct(self):
        title, _ = self.scene_title("What to trust", subtitle="A practical audit checklist")
        self.play(Write(title), run_time=RT_TITLE_IN)
        self.wait(0.35)
        ...
        rows = VGroup()
        for s in items:
            box = Rectangle(width=0.35, height=0.35, stroke_color=_color(PAL.muted), stroke_width=2)
        ...
        self.play(LaggedStart(*[FadeIn(r, shift=UP * 0.05) for r in rows], lag_ratio=0.08), run_time=RT_IN)

        checks = VGroup()
        for r in rows:
            check = txt("OK", size=20, color=PAL.green, font=FONT_MONO).move_to(r[0].get_center())
...
class Scene11_AlternativesTradeoffs(BaseTrustedSetupScene):
    def construct(self):
        title, _ = self.scene_title("Alternatives and tradeoffs", subtitle="Why people still use Groth16")
        self.play(FadeIn(title, shift=DOWN * 0.05), run_time=RT_TITLE_IN)
        self.wait(0.35)
        ...
        ).arrange(RIGHT, buff=0.35).shift(UP * 1.05)
